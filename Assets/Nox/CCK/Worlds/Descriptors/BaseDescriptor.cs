...
namespace Nox.CCK.Worlds
{
    ...
    public abstract class BaseDescriptor : MonoBehaviour
    {
        ...
        private static bool TryGetDescriptor<T>(GameObject root, out T descriptor) where T : BaseDescriptor
        {
        ...
        public static AudioChannel[] GetAudioChannels(Scene scene)
            => scene.GetRootGameObjects().Select(Finder.FindComponent<AudioChannel>).Where(channel => channel != null)
                .ToArray();
        ...
        private static T Choice<T>(T a, T b) => a == null ? b : a;
