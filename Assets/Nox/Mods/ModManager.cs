...
namespace Nox.Mods
{
    public class ModManager : Manager<RuntimeMod>
    {
        ...
        public static ModLoadResult[] LoadAllClientMods()
        {
            ...
            mods.Sort((a, b) =>
            {
                ...
                foreach (var required in a.GetMetadata().GetDepends())
                    if (required.GetId() == b.GetMetadata().GetId() || b.GetMetadata().GetProvides().Contains(required.GetId()))
                        i++;
                foreach (var required in b.GetMetadata().GetDepends())
                    if (required.GetId() == a.GetMetadata().GetId() || a.GetMetadata().GetProvides().Contains(required.GetId()))
                        i--;
                return i;
            });
            ...
            {
                foreach (var mod in mods) mod.Unload();
                return results.ToArray();
            }
        ...
        public static RuntimeMod GetMod(string id) => Cache.FirstOrDefault(m => m.GetMetadata().GetId() == id);
