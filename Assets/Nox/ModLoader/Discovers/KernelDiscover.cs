...
namespace Nox.ModLoader.Discovers
{
    public class KernelDiscover : IDiscover
    {
        ...
        public static IDiscover Instance => _instance ?? new KernelDiscover();
        ...
        public ModMetadata[] FindAllPackages()
        {
            ...
            foreach (var file in files)
            {
                ...
                var noxmod = Directory
                    .GetFiles(Path.GetDirectoryName(file), "nox.mod.json*", SearchOption.TopDirectoryOnly)
                    .FirstOrDefault();
                ...
                noxobj.InternalData["assets"] = Path.Combine(Path.GetDirectoryName(file), "assets");
 ...
 JObject.Parse(File.ReadAllText(Path.Combine(Application.dataPath, "Nox", "game_data.json")));
        ...
        public Mod CreateMod(ModMetadata metadata) => new KernelMod() { Metadata = metadata };
    }
}