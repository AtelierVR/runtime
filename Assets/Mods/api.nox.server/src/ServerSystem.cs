...
namespace api.nox.server
{
    public class ServerSystem : MainModInitializer
    {
        ...
        public void OnInitializeMain(MainModCoreAPI api)
        {
            ...
            _events = new[]
            {
                CoreAPI.EventAPI.Subscribe("server_update",
                    data => OnServerUpdated.Invoke(data.TryGet(0, out INoxObject server)
                        ? server
                        : null
                    )),
                CoreAPI.EventAPI.Subscribe("server_fetch",
                    data => OnServerFetched.Invoke(data.TryGet(0, out INoxObject server)
                        ? server
                        : null
                    )),
                CoreAPI.EventAPI.Subscribe("server_connect",
                    data => OnServerConnected.Invoke(data.TryGet(0, out INoxObject server)
                        ? server
                        : null
                    )),
                CoreAPI.EventAPI.Subscribe("server_disconnect",
                    data => OnServerDisconnected.Invoke(data.TryGet(0, out INoxObject server)
                        ? server
                        : null
                    ))
            };
