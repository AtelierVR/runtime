...
namespace api.nox.world.client
{
    public class WorldPage
    {
        ...
        {
            _listener = WorldSystem.CoreAPI.EventAPI.Subscribe("goto_page", OnGotoEvent);
        }

        public static void StopListen()
        {
            WorldSystem.CoreAPI.EventAPI.Unsubscribe(_listener);
        }
        ...
        private static void OnGotoEvent(EventData context)
        {
            ...
            switch (type)
            {
                case "id-server" when context.TryGet(3, out string id0) && context.TryGet(4, out uint ser0):
                    OnPageByIdentifier(menuId, WorldSystem.WorldAPI.CallMethod("IdentifierById", id0, ser0));
                    break;
                case "identifier" when context.TryGet(3, out string id2):
                    OnPageByIdentifier(menuId, WorldSystem.WorldAPI.CallMethod("IdentifierByString", id2));
                    break;
        ...
        private static void OnPageByIdentifier(int menuId, INoxObject identifier)
        {
            var page = new WorldPage
            {
                ...
                World = WorldSystem.WorldAPI.CallMethod("GetWorldByIdentifier", identifier)
            };
            page.Display();
            page.Refresh().Forget();
        }
        ...
        private void Display()
            => WorldSystem.CoreAPI.EventAPI.Emit("display_page", MenuId, new Dictionary<string, object>
            {
                {
                    "key", GetKey()
                }, // id of the page
        ...
        {
            var asset = WorldSystem.CoreAPI.AssetAPI.GetAsset<GameObject>("prefabs/content.prefab");
        ...
        internal void GoBack()
            => WorldSystem.CoreAPI.EventAPI.Emit("goto_action", MenuId, "back");
