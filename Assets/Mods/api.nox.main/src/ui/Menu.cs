...
namespace api.nox.game.UI
{
    public class Menu : MonoBehaviour, ShareObject, IDisposable
    {
        ...
        public void SetTile(TileObject tile, TileObject oldTile = null, SetTileFlags flags = SetTileFlags.None)
        {
            try
            {
                if (!tile.content && tile.GetContent == null)
                    return;

                if (oldTile != null)
                {
                    Logger.Log($"Hiding old tile {oldTile.id}");
                    oldTile.onHide?.Invoke(tile.id);
                ...
                if (flags.HasFlag(SetTileFlags.IsNew))
                {
                    ...
                    tile.onOpen?.Invoke(oldTile?.id);
                }
                
                if (flags.HasFlag(SetTileFlags.IsRestore))
                {
                    Logger.Log($"Restoring tile {tile.id}");
                    tile.onRestore?.Invoke(oldTile?.id);
                }
                
                Logger.Log($"Displaying tile {tile.id}");
                tile.onDisplay?.Invoke(oldTile?.id, tile.content);
