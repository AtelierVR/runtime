...
namespace api.nox.network.Utils
{
    public class Gateway
    {
        ...
        public static async UniTask<SRVAnswer[]> FindSRV(string domain, string service, string protocol = "tcp")
        {
            try
            {
                Logger.Log($"https://dns.google/resolve?name={string.Format(service, domain)}&type=SRV");
                var req = new UnityWebRequest($"https://dns.google/resolve?name={string.Format(service, domain, protocol)}&type=SRV", UnityWebRequest.kHttpVerbGET) { downloadHandler = new DownloadHandlerBuffer() };
