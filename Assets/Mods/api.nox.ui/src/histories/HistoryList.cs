...
namespace api.nox.ui.histories {
	public class HistoryList {
		...
		public void Add(IPage page) {
			...
			_menu.SetPage(page, crt, PageFlags.IsNew | PageFlags.IsForward);
		}
		...
		public void GoBack(int count = 1) {
			...
			while (count-- > 0 && _current > 0)
				_current--;
			_menu.SetPage(_cache[_current], crt, PageFlags.IsRestore | PageFlags.IsBack);
		}
		...
		public void GoForward(int count = 1) {
			...
			while (count-- > 0 && _current < _cache.Count - 1)
				_current++;
			_menu.SetPage(_cache[_current], crt, PageFlags.IsRestore | PageFlags.IsForward);
		}
		...
		private void RemoveRange(int v1, int v2) {
			...
			var cur = GetCurrent();
			if (old != cur)
				_menu.SetPage(cur, old, PageFlags.IsRestore | PageFlags.IsBack);
		}
		...
		public void Clear() {
			...
			_current = -1;
			_menu.SetPage(null, crt, PageFlags.IsBack);
		}

		internal void Restore() {
			var crt = GetCurrent();
			_menu.SetPage(crt, crt, PageFlags.IsRestore);
		}
	}
}