...
namespace Hactazia.FFPlay.Helpers {
	public sealed unsafe class Context : IDisposable {
		private readonly  object                         _readFrameLock = new object();
		...
		public Context(Stream stream, uint bufferSize = 16_000_000) {
			...
			ffmpeg.avformat_open_input(&pFormatContext, url, null, null).ThrowFFmpegException();
			ffmpeg.avformat_find_stream_info(_pFormatContext, null).ThrowFFmpegException();
		...
		public bool TryGetFps(Decoder decoder, out double fps) {
			fps = default;
			int _streamIndex = decoder._streamIndex;
			if (_streamIndex < 0 || _streamIndex > _pFormatContext->nb_streams)
				return false;
		...
		public bool TryGetPts(Decoder decoder, out double fps) {
			fps = default;
			int _streamIndex = decoder._streamIndex;
			if (_streamIndex < 0 || _streamIndex > _pFormatContext->nb_streams)
				return false;
		...
		public bool TryGetTimeBase(AVMediaType type, out AVRational timebase) {
			timebase = default;
			int _streamIndex = ffmpeg.av_find_best_stream(_pFormatContext, type, -1, -1, null, 0).ThrowFFmpegException();
			if (_streamIndex < 0 || _streamIndex > _pFormatContext->nb_streams)
				return false;
		...
		public bool TryGetTime(Decoder decoder, out double time) {
			time = default;
			int _streamIndex = decoder._streamIndex;
			if (_streamIndex < 0 || _streamIndex > _pFormatContext->nb_streams)
				return false;
		...
		public bool TryGetTime(Decoder decoder, AVFrame frame, out double time) {
			time = default;
			int _streamIndex = decoder._streamIndex;
			if (_streamIndex < 0 || _streamIndex > _pFormatContext->nb_streams)
				return false;
		...
		public bool NextFrame(out AVPacket packet) {
			...
			do {
				...
				// Handle TLS/network errors with retry
				if (error < 0) {
					var errorDesc = error.FFmpegDescribe();
					if (errorDesc.Contains("tls", StringComparison.OrdinalIgnoreCase) || errorDesc.Contains("Connection", StringComparison.OrdinalIgnoreCase) || errorDesc.Contains("I/O", StringComparison.OrdinalIgnoreCase)) {
						retryCount++;
		...
		public void Seek(int index, long offset) {
			...
			ffmpeg.av_seek_frame(_pFormatContext, index, offset, flags)
				.ThrowFFmpegException();
		}

		public void Seek(Decoder decoder, double offset) {
			if (!IsValid)
				return;
			int        _streamIndex = decoder._streamIndex;
