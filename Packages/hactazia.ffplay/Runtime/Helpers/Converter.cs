...
namespace Hactazia.FFPlay.Helpers {
	public sealed unsafe class Converter : IDisposable {
		...
		private readonly List<byte_ptrArray4> ptrs  = new();
		private readonly List<IntPtr>         ptrs2 = new();

		public Converter(Size sourceSize,      AVPixelFormat sourcePixelFormat,
		...
		public AVFrame Convert(AVFrame sourceFrame, int align = -1) {
			...
			if (align < 0) {
				for (uint i = 1; i <= 64; i *= 2) {
					if (Mathf.Abs(sourceFrame.linesize[0 * i]) % i == 0 && Mathf.Abs(sourceFrame.linesize[1 * i]) % i == 0 && Mathf.Abs(sourceFrame.linesize[2 * i]) % i == 0) { } else {
						j = (int)i;
						break;
					}
				}
