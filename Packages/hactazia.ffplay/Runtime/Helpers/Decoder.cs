...
namespace Hactazia.FFPlay.Helpers {
	public sealed unsafe class Decoder : IDisposable {
		...
		public Decoder(Context ctx, AVMediaType type, AVHWDeviceType HWDeviceType = AVHWDeviceType.AV_HWDEVICE_TYPE_NONE) {
			_ctx = ctx;
			AVCodec* codec = null;
			_streamIndex = ffmpeg
			...
			Codec               = ffmpeg.avcodec_alloc_context3(codec);
			Codec->thread_count = 0;

			if (HWDeviceType != AVHWDeviceType.AV_HWDEVICE_TYPE_NONE) {
				get_hw_fmt        = GetHWFormat;
				Codec->get_format = get_hw_fmt;
			}

			ffmpeg.avcodec_parameters_to_context(Codec, _ctx._pFormatContext->streams[_streamIndex]->codecpar)
				.ThrowFFmpegException();
			...
			if (HWDeviceType != AVHWDeviceType.AV_HWDEVICE_TYPE_NONE) {
				...
				Codec->hw_device_ctx = ffmpeg.av_buffer_ref(hw_device_ctx);
			}

			ffmpeg.avcodec_open2(Codec, codec, null).ThrowFFmpegException();
		...
		public int DecodeNonAlloc(ref AVFrame frame) {
			...
			int error2;
			do {
				error  = ffmpeg.avcodec_send_packet(Codec, _ctx._pPacket);
				error2 = ffmpeg.avcodec_receive_frame(Codec, _pFrame);
			} while (error == ffmpeg.AVERROR(ffmpeg.EAGAIN));

			// Handle invalid/corrupt data gracefully - common in streaming
			...
			if (error2 == ffmpeg.AVERROR_INVALIDDATA) {
				return -1; // Skip corrupt frame
			}

			error2.ThrowFFmpegException();
			if (Codec->hw_device_ctx != null) {
				_receivedFrame->hw_frames_ctx = Codec->hw_device_ctx;
				ffmpeg.av_hwframe_transfer_data(_receivedFrame, _pFrame, 0).ThrowFFmpegException();
		...
		public int Decode(out AVFrame frame) {
			...
			// Handle invalid/corrupt data gracefully - common in streaming
			if (error == ffmpeg.AVERROR_INVALIDDATA) {
				frame = new AVFrame() { format = -1 };
				return 1; // Skip this packet
			}

			// Handle EOF gracefully - it's a normal end condition, not an error
			...
			// Handle EOF gracefully in receive as well
		...
		public void Seek() {
			...
			// ffmpeg.avcodec_send_packet(_pCodecContext, null).ThrowFFmpegException();
		}
		...
		public void Seek(long offset) {
			...
			// ffmpeg.avformat_seek_file(_pFormatContext, _streamIndex, 0, target, long.MaxValue, ffmpeg.AVFMT_SEEK_TO_PTS).ThrowFFmpegException();
			ffmpeg.av_seek_frame(_ctx._pFormatContext, _streamIndex, offset, ffmpeg.AVSEEK_FLAG_FRAME).ThrowFFmpegException();
		}
