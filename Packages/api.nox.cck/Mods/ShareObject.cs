            ...
            foreach (var prop in GetType().GetFields().Where(f => f.GetCustomAttributes(typeof(ShareObjectImportAttribute), false).Length > 0))
                if (dict.ContainsKey(prop.Name))
                    if (dict[prop.Name] != null)
                    {
                        ...
                        var pInterfaces = prop.FieldType.GetInterfaces();
                        var vInterfaces = value.GetType().GetInterfaces();
                        if (pInterfaces.Contains(typeof(ShareObject)) && vInterfaces.Contains(typeof(ShareObject)))
                            prop.SetValue(this, (value as ShareObject)?.Convert(prop.FieldType));
                        else if (value is Dictionary<string, object> objects && pInterfaces.Contains(typeof(ShareObject)))
                        {
                            var propValue = (ShareObject)Activator.CreateInstance(prop.FieldType);
                            propValue.Import(objects);
