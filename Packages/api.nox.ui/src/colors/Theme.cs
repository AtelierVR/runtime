...
namespace api.nox.ui.colors {
	...
	public class Theme : ScriptableObject {
		...
		public Dictionary<string, Dictionary<string, string>> ToDictionary() {
			...
			foreach (var group in colors) {
				...
				foreach (var color in group.colors) {
					colorDict[color.name] = color.value;
				}

				dict[group.name] = colorDict;
			}
		...
		public bool TryGetColor(string name, string value, out UColor color)
			=> TryGetColor(ToDictionary(), name, value, out color);

		public static bool TryGetColor(
			Dictionary<string, Dictionary<string, string>> colors, string name, string value, out UColor color) {
			if (colors.TryGetValue(name, out var colorDict)
			    && colorDict.TryGetValue(value, out var hex)
			    && ColorUtility.TryParseHtmlString(hex, out var colorParsed)) {
				color = colorParsed;
				return true;
			}
