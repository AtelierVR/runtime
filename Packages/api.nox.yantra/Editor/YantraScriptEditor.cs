...
namespace api.nox.yantra {
	...
	[CustomEditor(typeof(YantraScript))]
	public class YantraScriptEditor : Editor {
		public YantraScript Module
			=> (YantraScript)target;
		...
		public override VisualElement CreateInspectorGUI() {
			var iconAsset = Resources.Load<Texture2D>("api.nox.yantra.yantrascript");
			if (iconAsset) EditorGUIUtility.SetIconForObject(target, iconAsset);
			var inspectorAsset = Resources.Load<VisualTreeAsset>("api.nox.yantra.yantrascript");
			...
			sc.RegisterValueChangedCallback(
				evt => {
					if (evt.newValue is not YantraFile newScript) return;
