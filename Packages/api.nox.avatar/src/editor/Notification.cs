...
namespace api.nox.avatar.editor {
	public class NotificationManager {
		...
		public static void Remove(string uid)
			=> GetMany(uid).ForEach(n => Remove(n));

		public static Notification Get(string uid)
			=> Notifications.Find(n => n.Uid == uid);

		public static List<Notification> GetMany(string uid) {
			return uid.StartsWith("*") ? Notifications.FindAll(n => n.Uid.EndsWith(uid[1..]))
				: uid.EndsWith("*")    ? Notifications.FindAll(n => n.Uid.StartsWith(uid[..^1]))
