...
namespace api.nox.avatar {
	public class AssetAvatar : BaseAvatar {
		public (string ns, string path) Path;

		public static async UniTask<AssetAvatar> Load(string ns, string path, Action<float> progress, CancellationToken token) {
			progress?.Invoke(0);

			var avatar = new AssetAvatar {
				Path = (ns, path)
			};
