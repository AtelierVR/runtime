...
namespace api.nox.avatar {
	public class AssetBundleAvatar : BaseAvatar {
		public AssetBundle Bundle;
		public string      Path;

		public bool CanUnloadAssetBundle() {
			foreach (var a0 in AvatarLoader.Avatar)
				if (a0 is AssetBundleAvatar a1 && a1.Path == Path && a1.GetId() != GetId())
					return false;
			return true;
		}
		...
		private static AssetBundle GetAssetBundle(string path) {
			foreach (var a0 in AvatarLoader.Avatar)
				if (a0 is AssetBundleAvatar a1 && a1.Path == path)
					return a1.Bundle;
			return null;
		}


		public static async UniTask<AssetBundleAvatar> Load(string path, Action<float> progress, CancellationToken token) {
			progress?.Invoke(0);

			var avatar = new AssetBundleAvatar {
				Path   = path,
