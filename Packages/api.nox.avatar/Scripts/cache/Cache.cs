...
namespace api.nox.avatar.cache {
	public class Cache {
		public Cache() {
		...
		private async UniTask OnFileAsync(FileSystemEventArgs e) {
			...
			if (e.ChangeType == WatcherChangeTypes.Created)
				Main.Instance.CoreAPI.EventAPI.Emit("avatar_cache_added", path);
			else if (e.ChangeType == WatcherChangeTypes.Deleted)
				Main.Instance.CoreAPI.EventAPI.Emit("avatar_cache_removed", path);
		}
		...
		private async UniTask OnRenameFileAsync(RenamedEventArgs e) {
			...
			Main.Instance.CoreAPI.EventAPI.Emit("avatar_cache_removed", oldPath);
			Main.Instance.CoreAPI.EventAPI.Emit("avatar_cache_added", newPath);
		}
		...
		public Caching AddDownload(string url, string hash = null, CancellationToken token = default) {
			var existing = GetDownload(url, hash);
			return existing ?? new Caching(this, url, hash, token);
		}

		public static string CachePath {
			get {
				var dir = Path.Combine(Constants.CachePath, "avatars");
